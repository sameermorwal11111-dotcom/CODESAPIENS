...
- Add new face data
- Train classifier
- Live face recognition with name display
- All in one script
...
class FaceRecognitionSystem:
    ...
    def capture_face_data(self, name, num_images=50):
        ...
        while True:
            ...
            cv2.imshow(f"Capturing Face Data - {name.upper()}", frame)
    ...
    def train_classifier(self, name):
        ...
        for img_file in image_files:
            try:
                ...
                if img is not None:
                    images.append(img)
                    labels.append(0)  # All images of same user have label 0
    ...
    def recognize_faces_live(self, name, timeout=15):
        ...
        while True:
            ...
            # Draw info panel
            cv2.rectangle(frame, (0, 0), (500, 100), (0, 0, 0), -1)
            ...
            for (x, y, w, h) in faces:
                ...
                try:
                    ...
                    if confidence_percent > 50:
                        matched = True
                        label = f"{BOLD}MATCH{RESET}: {name.upper()} ({confidence_percent}%)"
                        color = (0, 255, 0)  # Green
                        label_color = (0, 255, 0)
                    else:
...
def print_menu():
    """Print main menu"""
    print(f"\n{BOLD}{CYAN}╔════════════════════════════════════════════════════════╗{RESET}")
    print(f"{BOLD}{CYAN}║        FACE RECOGNITION COMPLETE SYSTEM               ║{RESET}")
